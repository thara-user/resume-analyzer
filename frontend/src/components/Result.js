export default function Result({ result, onReset }) {
  ...
  const label = result.score >= 70 ? "Excellent! 💪" : result.score >= 40 ? "Good — Keep Learning 📚" : "Beginner — Keep Going 🚀";
  return (
    <div style={{background:"#1e293b",borderRadius:20,padding:32}}>
      ...
      <div style={{marginBottom:24}}>
        <div style={{background:"#0f172a",borderRadius:99,height:12,overflow:"hidden"}}>
          <div style={{width:result.score+"%",height:"100%",background:color,borderRadius:99}} />
        </div>
      </div>
      <div style={{marginBottom:20}}>
        <p style={{color:"#fff",fontWeight:700,marginBottom:10}}>✅ Matched Skills ({result.matched.length})</p>
        <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
          {result.matched.length > 0 ? result.matched.map(k => (
      ...
      {result.missing.length > 0 && (
        <div style={{marginBottom:24}}>
          <p style={{color:"#fff",fontWeight:700,marginBottom:10}}>❌ Missing Skills ({result.missing.length})</p>
          <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
